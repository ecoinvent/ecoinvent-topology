...
class EcoinventChecker(object):
        ...
        with open(log_fp, "w") as logfile:
            ...
            for key in config:
                if key not in ecoinvent:
                    logfile.write(
                        "{}: {}, {}\n".format(
                            key, config[key]["name"], config[key]["shortname"]
                        )
                    )
            ...
            for key in ecoinvent:
                if key not in config:
                    logfile.write(
                        "{}: {}, {}\n".format(
                            key, ecoinvent[key]["name"], ecoinvent[key]["shortname"]
                        )
                    )
            ...
            for key in all_keys:
                if ecoinvent[key]["shortname"] != config[key]["shortname"]:
                    logfile.write(
                        "{}: {}\n".format(
                            config[key]["shortname"], ecoinvent[key]["shortname"]
                        )
                    )
    ...
    def extract_ecoinvent_data(self, fp):
        ...
        for obj in root.iterchildren():
            ...
            data.append(
                {"id": obj.get("id"), "name": name.text, "shortname": shortname.text}
            )
        return data
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Check UUIDs and names against ecoinvent master data."
    )
    parser.add_argument(
        "filepath", type=str, help="`Geographies.xml` filepath from EcoEditor"
    )
