...
class Ecospold2GeoExporter(object):
    ...
    def __init__(self, output="Geographies.xml", languages=('es', 'ja', 'fr', 'de', 'zh')):
        self.output_fp = os.path.realpath(os.path.join(
            os.path.abspath(os.path.dirname(__file__)), "..", "output", output
        ))
        self.input_fp = os.path.realpath(os.path.join(
    ...
    def create_document(self):
        G = ElementMaker(namespace="http://www.EcoInvent.org/EcoSpold02",
    ...
    def get_element(self, geo, G):
        ...
        for language in self.languages:
            if data['ISOTwoLetterCode'] in self.language_dictionary[language]:
                ELEMS.append(G.name(self.language_dictionary[language][data['ISOTwoLetterCode']],
                    {"{%s}lang" % XML_NS: language}))

        ELEMS += [
            G.name(data[u'name'], {u"{%s}lang" % XML_NS: u"en"}),
            G.shortname(data[u'shortname'], {u"{%s}lang" % XML_NS: u"en"}),
        ]

        # Add description is available
        # if geo.description:
        #     ELEMS.append(
        #         G.comment(
