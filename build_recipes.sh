...
psql -U ecoinvent -d eigeo -c "INSERT INTO final (collection, name, shortname, geom) (SELECT 'states', 'United States of America, ' || g.name, n.iso_3166_2, geometry(g.topogeom) FROM geometries g LEFT JOIN ne_provinces n ON g.gid = n.gid WHERE g.tname = 'ne_provinces' AND g.parent = 'United States of America' AND g.name IS NOT NULL);" -q -n -o create_db.log

echo "Add Australian states"
psql -U ecoinvent -d eigeo -c "INSERT INTO final (collection, name, shortname, geom) (SELECT 'states', 'Australia, ' || g.name, n.code_hasc, geometry(g.topogeom) FROM geometries g LEFT JOIN ne_provinces n ON g.gid = n.gid WHERE g.tname = 'ne_provinces' AND g.parent = 'Australia' AND g.name IS NOT NULL AND g.name in ('Northern Territory', 'Western Australia', 'Australian Capital Territory', 'New South Wales', 'South Australia', 'Victoria', 'Queensland', 'Tasmania'));" -q -n -o create_db.log

echo "Add Brazilian states"
psql -U ecoinvent -d eigeo -c "INSERT INTO final (collection, name, shortname, geom) (SELECT 'states', 'Brazil, ' || g.name, n.code_hasc, geometry(g.topogeom) FROM geometries g LEFT JOIN ne_provinces n ON g.gid = n.gid WHERE g.tname = 'ne_provinces' AND g.parent = 'Brazil' AND g.name IS NOT NULL AND g.name != '');" -q -n -o create_db.log

echo "Add Indian states and union territories"
psql -U ecoinvent -d eigeo -c "INSERT INTO final (collection, name, shortname, geom) (SELECT 'states', 'India, ' || g.name, n.code_hasc, geometry(g.topogeom) FROM geometries g LEFT JOIN ne_provinces n ON g.gid = n.gid WHERE g.tname = 'ne_provinces' AND g.parent = 'India' AND g.name IS NOT NULL AND g.name != '');" -q -n -o create_db.log
